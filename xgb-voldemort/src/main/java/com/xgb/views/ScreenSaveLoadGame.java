...
import com.xgb.scene.SaveLoadGame;
import com.xgb.script.ScriptResources;
...
public class ScreenSaveLoadGame extends BaseScreen {
	...
	public ScreenSaveLoadGame(Operate opt) {
		...
		mHeadImgs.add(new ArrayList<ResImage>());
		mHeadImgs.add(new ArrayList<ResImage>());
		File file =new File(System.getProperty("user.dir")+"/xgb-voldemort/src/main/java/com/xgb/assets/"+mFileNames[0]);//GameActivity.instance.getFileStreamPath(mFileNames[0]);
		
		if (file.exists()) {
			mText[0] = format(getSceneNameAndHeads(file, mHeadImgs.get(0)));
		}
		file = new File(System.getProperty("user.dir")+"/xgb-voldemort/src/main/java/com/xgb/assets/"+mFileNames[1]);//GameActivity.instance.getFileStreamPath(mFileNames[1]);
		if (file.exists()) {
			mText[1] = format(getSceneNameAndHeads(file, mHeadImgs.get(1)));
		}
		file = new File(System.getProperty("user.dir")+"/xgb-voldemort/src/main/java/com/xgb/assets/"+mFileNames[2]);//GameActivity.instance.getFileStreamPath(mFileNames[2]);
		if (file.exists()) {
	...
	public void onKeyUp(int key) {
		if (key == Global.KEY_CANCEL) {
			...
			final File file = new File(System.getProperty("user.dir")+"/xgb-voldemort/src/main/java/com/xgb/assets/"+mFileNames[mIndex]);//GameActivity.instance.getFileStreamPath(mFileNames[mIndex]);
